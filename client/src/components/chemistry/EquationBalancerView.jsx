        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-900 tracking-tight flex items-center gap-2">
              <span>{t('balancer.title', 'Linear Algebraic Chemical Equation Balancer')}</span>
              <span className="rounded-full bg-amber-100 text-amber-800 px-2 py-0.5 text-[10px] font-black">
                {t('balancer.massConserved', 'Conservation of Mass')}
              </span>
            </h2>
            <p className="text-xs text-slate-500">
              {t('balancer.subtitle', 'Balances stoichiometric coefficients using linear matrix nullspace and verifies element atom counts.')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-xs">
        <div>
          <label className="text-xs font-black uppercase text-slate-400">
            {t('balancer.enterEquation', 'Enter Unbalanced Chemical Equation:')}
          </label>
          <div className="mt-1.5 flex flex-col sm:flex-row items-center gap-2">
            ...
            >
              <Sparkles size={15} />
              <span>{t('balancer.balanceButton', 'Balance Equation')}</span>
            </button>
          </div>
        </div>
        ...
        {/* Quick Presets */}
        <div>
          <span className="text-[11px] font-bold uppercase text-slate-400">{t('balancer.cbsePresets', 'Iconic CBSE Equation Presets:')}</span>
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-xs font-black uppercase text-emerald-700 flex items-center gap-1.5">
                    ...
                    <span>{t('balancer.balancedHero', 'Balanced Equation (Stoichiometrically Exact)')}</span>
                  </span>
                  <span className="rounded-lg bg-emerald-100 text-emerald-800 px-2.5 py-0.5 text-[10px] font-black">
                    {t('balancer.massConserved', 'Mass Conserved')}
                  </span>
                </div>
              ...
              {balanceResult.atomCounts && (
                <div>
                  <h4 className="text-xs font-black uppercase text-slate-400 mb-2">
                    {t('balancer.atomAudit', 'Atomic Species Conservation Audit:')}
                  </h4>
                  <div className="overflow-x-auto rounded-2xl border border-slate-200">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold">
                        <tr>
                          <th className="p-2.5 pl-4">{t('balancer.element', 'Element')}</th>
                          <th className="p-2.5 text-center">{t('balancer.reactantSide', 'Reactant Side (Before)')}</th>
                          <th className="p-2.5 text-center">{t('balancer.productSide', 'Product Side (After)')}</th>
                          <th className="p-2.5 text-center">{t('balancer.balanceStatus', 'Balance Status')}</th>
                        </tr>
                      </thead>
                          ...
                          <tr key={elem} className="hover:bg-slate-50/60">
                            ...
                            <td className="p-2.5 text-center">
                              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black text-emerald-800">
                                <Check size={10} /> {t('balancer.equal', 'Equal')}
                              </span>
                            </td>
                          </tr>
                        ))}
              ...
              {balanceResult.steps && (
                <div className="pt-2 border-t border-slate-100">
                  <h4 className="text-xs font-black uppercase text-slate-400 mb-2 flex items-center gap-1.5">
                    ...
                    <span>{t('balancer.stepDerivation', 'Step-by-Step Linear Algebraic Resolution:')}</span>
                  </h4>
